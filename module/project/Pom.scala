...
object Pom {
  ...
  def name(pomDir: File): String = {
    val pom = new File(pomDir, "pom.xml")
    val xml = XML.loadFile(pom)
    val name = (xml \\ "project" \ "name").text
    name
  }
  def scalaVersion(pomDir: File): String = {
    val pom = new File(pomDir, "pom.xml")
    val xml = XML.loadFile(pom)
    val scalaVersion = (xml \\ "project" \\ "properties" \ "scala.version").text
    scalaVersion
  }
  def playVersion(pomDir: File): String = {
    val pom = new File(pomDir, "pom.xml")
    val xml = XML.loadFile(pom)
    val playVersion = (xml \\ "project" \\ "properties" \ "play2.version").text
    playVersion
  }
